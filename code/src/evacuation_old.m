function [agents_info bed_coords nb_patient_curr nb_bed_patient_curr nb_patient_tot_curr nb_staff_curr]=evacuation_old(num_agents,perc_staff, perc_beds, image_name, real_time_display)
...
%[staff_ordered_to_bed,bed_free_coords]=attribuate_bed(agent,bed_coords);
...
%agents_info(3,1:nb_staff,1)=bed_coords(1,1:nb_staff); % x bed coord
%agents_info(4,1:nb_staff,1)=bed_coords(2,1:nb_staff); % y bed coord
...
for time=1:tf
    ...
    for i=1:nb_staff;
        ...
        if (distance_staff_exit(i)<3 && agents(6,i)==2 && size(bed_free_coords,2)==0 )
            ...
            agents(6,i)=3;
            bed_e_x_attribuated(:,:,i)=0;
            bed_e_y_attribuated(:,:,i)=0;
            stop(i)=0;
        ...
        if (distance_staff_exit(i)<3 && agents(6,i)==2 && size(bed_free_coords,2)~=0 )
            ...
            agents(6,i)=4;
            [temp1 temp2]=gradientmap(wall_map,bed_free_coords(:,1));
            %temp1 & temp2 - temporary values for position vectors
            bed_e_x_attribuated(:,:,i)=temp1;
            bed_e_y_attribuated(:,:,i)=temp2;
    ...
    distance_patient_exit = zeros(nb_exit,nb_patient_curr(time));
    ...
    for i= nb_staff+1:size(agents,2)
        for l=1:nb_exit
            distance_patient_exit(l,i)=distance_alpha_beta(agents(1,i),agents(2,i),exit_coords(2,l),exit_coords(1,l));
            if (distance_patient_exit(l,i)<= 2)
                counter_patient_outside = counter_patient_outside+1;
                index_patient_outside(counter_patient_outside)=i; % saved patient
            end
        end
    end
    if (counter_patient_outside>1)
        remove_count=0;
        for i=1:counter_patient_outside
            for j=i+1:counter_patient_outside
                %if (index_patient_outside(i)==index_patient_outside(j))
                index_patient_outside(j)=[];
                counter_patient_outside=counter_patient_outside-1;
                %end
            end
        end
    ...
    if (counter_patient_outside~=0)
        for i=1:counter_patient_outside
            agents(:,index_patient_outside(i))=[];
...
%figure,
%plot(time_history(1:time),nb_patient_curr(1:time),'r--',time_history(1:time),nb_bed_patient_curr(1:time),'r-.',time_history(1:time),nb_patient_tot_curr(1:time),'r-',time_history(1:time),nb_staff_curr(1:time),'b-');
